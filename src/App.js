...
function AuthPage({ page, setPage, showToast }) {
  ...
  const handleRegister = async () => {
    ...
    try {
      ...
      if (role === "admin") { await setDoc(doc(db, "settings", "accounts"), { list: DEFAULT_ACCOUNTS }); }
      showToast("登録完了！");
...
function MainLayout({ profile, setProfile, user, page, setPage, logout, showToast }) {
  ...
  const nav = [{ id: "dashboard", icon: "📊", label: "ダッシュボード" },{ id: "journal", icon: "📒", label: "仕訳入力" },{ id: "ledger", icon: "📖", label: "仕訳帳" },{ id: "invoices", icon: "📄", label: "請求書" },{ id: "reports", icon: "📈", label: "レポート" },{ id: "receipts", icon: "🧾", label: "領収書" },{ id: "accounts", icon: "🏷️", label: "勘定科目" },{ id: "members", icon: "👥", label: "メンバー" },{ id: "settings", icon: "⚙️", label: "設定" }];
  return (
    <div style={{ display: "flex", minHeight: "100vh" }}>
      ...
      <div style={{ flex: 1, padding: 28, overflowY: "auto", maxHeight: "100vh" }}>
        <div style={{ maxWidth: 960, margin: "0 auto", animation: "fadeIn .3s ease" }}>
          ...
          {page === "members" && <MembersPage showToast={showToast} />}
...
/* ══════════════════ MEMBERS ══════════════════ */
function MembersPage({ showToast }) {
  const [members, setMembers] = useState([]); const [loading, setLoading] = useState(true);
  const { isAdmin } = useRole();
  useEffect(() => { (async () => { setLoading(true); const s = await getDocs(collection(db, "users")); const a = []; s.forEach(d => a.push({ id: d.id, ...d.data() })); setMembers(a.sort((a, b) => (b.createdAt?.seconds || 0) - (a.createdAt?.seconds || 0))); setLoading(false); })(); }, []);
  const toggleRole = async (m) => {
    ...
    showToast(`${m.email} を${newRole === "admin" ? "管理者" : "一般メンバー"}に変更しました`);
    setMembers(members.map(x => x.id === m.id ? { ...x, role: newRole } : x));
  };
  return (<div><PageTitle sub={`${members.length}名が登録済み`}>メンバー</PageTitle>
    {loading ? <p style={{ color: "#64748B" }}>読み込み中...</p> : <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>{members.map(m => <Card key={m.id} style={{ padding: 16 }}><div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}><div><div style={{ display: "flex", alignItems: "center", gap: 8 }}><span style={{ fontSize: 14, fontWeight: 600, color: "#E2E8F0" }}>{m.companyName || m.email}</span><span style={{ background: m.role === "admin" ? "rgba(16,185,129,.15)" : "rgba(59,130,246,.15)", color: m.role === "admin" ? "#10B981" : "#60A5FA", fontSize: 10, padding: "1px 8px", borderRadius: 4, fontWeight: 600 }}>{m.role === "admin" ? "管理者" : "閲覧"}</span></div><div style={{ fontSize: 12, color: "#64748B", marginTop: 2 }}>{m.email} · 登録: {fmtDate(m.createdAt)}</div></div>{isAdmin && <Btn variant="ghost" onClick={() => toggleRole(m)} style={{ fontSize: 11, padding: "4px 12px" }}>{m.role === "admin" ? "閲覧に変更" : "管理者に変更"}</Btn>}</div></Card>)}</div>}
  </div>);
}
