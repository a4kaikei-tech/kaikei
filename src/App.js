...
function InvoicesPage({ user, profile, showToast }) {
  ...
  const updateStatus = async (inv, status) => {
    ...
    try {
      ...
      if (status === "paid") {
        ...
        showToast(`${selectedAccount}として支出登録完了！`);
      } else {
  ...
  const deleteInv = async (id) => {
    if (!isAdmin || !window.confirm("削除してもよろしいですか？")) return;
  ...
  return (
    <div>
        ...
        <Card style={{ marginBottom: 24, animation: "fadeIn .3s ease" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
              <div>
                <label style={{ fontSize: 12, color: "#94A3B8", marginBottom: 6, display: "block" }}>買い出し先</label>
                <input style={inputBase} value={form.client} onChange={e => setForm({ ...form, client: e.target.value })} placeholder="〇〇店" />
              </div>
              <div>
                <label style={{ fontSize: 12, color: "#94A3B8", marginBottom: 6, display: "block" }}>BLK会議の日程</label>
                <input type="date" style={{ ...inputBase, colorScheme: "dark" }} value={form.dueDate} onChange={e => setForm({ ...form, dueDate: e.target.value })} />
              </div>
            </div>

            <div>
              <label style={{ fontSize: 12, color: "#94A3B8", marginBottom: 8, display: "block" }}>明細</label>
              {form.items.map((item, i) => (
                <div key={i} style={{ display: "flex", gap: 8, marginBottom: 8, alignItems: "center" }}>
                  <input style={{ ...inputBase, flex: 3 }} placeholder="品目" value={item.name} onChange={e => updateItem(i, "name", e.target.value)} />
                  <input style={{ ...inputBase, flex: 1 }} type="number" placeholder="個数" value={item.qty} onChange={e => updateItem(i, "qty", e.target.value)} />
              ...
              <button onClick={addItem} style={{ background: "none", border: "none", color: "#34D399", cursor: "pointer", fontSize: 13 }}>＋ 明細を追加</button>
            </div>

            <div style={{ display: "flex", justifyContent: "flex-end", borderTop: "1px solid #334155", paddingTop: 16 }}>
              <div style={{ textAlign: "right", fontSize: 13 }}>
                <div style={{ color: "#94A3B8" }}>小計: {fmtYen(subtotal)}</div>
                <div style={{ color: "#94A3B8" }}>消費税 (10%): {fmtYen(tax)}</div>
                <div style={{ color: "#F1F5F9", fontWeight: 700, fontSize: 18, marginTop: 4 }}>合計: {fmtYen(total)}</div>
              </div>
            </div>
      ...
      {loading ? (
        ...
        <Card><p style={{ color: "#475569", textAlign: "center", padding: 20 }}>申請データがありません</p></Card>
      ) : (
              ...
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", flexWrap: "wrap", gap: 12 }}>
                <div>
                  <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 6 }}>
                    <span style={{ fontSize: 14, fontWeight: 700, color: "#F1F5F9" }}>{inv.companyName}</span>
                    <StatusBadge status={inv.status} />
                  </div>
                  <div style={{ fontSize: 13, color: "#34D399", fontWeight: 600 }}>{inv.client}</div>
                ...
                <div style={{ textAlign: "right" }}>
                  ...
                  {isAdmin && (
                    <div style={{ display: "flex", alignItems: "center", gap: 8, marginTop: 12, flexWrap: "wrap", justifyContent: "flex-end" }}>
                      {inv.status === "draft" && (
