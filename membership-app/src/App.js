...
const DEFAULT_ACCOUNTS = [
  { id: "sales", name: "売上高", type: "income", group: "売上" },
  { id: "service_income", name: "サービス収入", type: "income", group: "売上" },
  { id: "other_income", name: "雑収入", type: "income", group: "売上" },
  { id: "purchases", name: "仕入高", type: "expense", group: "売上原価" },
  { id: "salary", name: "給与手当", type: "expense", group: "販管費" },
  { id: "rent", name: "地代家賃", type: "expense", group: "販管費" },
  { id: "utilities", name: "水道光熱費", type: "expense", group: "販管費" },
  { id: "communication", name: "通信費", type: "expense", group: "販管費" },
  { id: "transport", name: "旅費交通費", type: "expense", group: "販管費" },
  { id: "supplies", name: "消耗品費", type: "expense", group: "販管費" },
  { id: "entertainment", name: "接待交際費", type: "expense", group: "販管費" },
  { id: "advertising", name: "広告宣伝費", type: "expense", group: "販管費" },
  { id: "insurance", name: "保険料", type: "expense", group: "販管費" },
  { id: "depreciation", name: "減価償却費", type: "expense", group: "販管費" },
  { id: "tax", name: "租税公課", type: "expense", group: "販管費" },
  { id: "misc_expense", name: "雑費", type: "expense", group: "販管費" },
];

const INVOICE_STATUS = {
  draft: { bg: "#334155", text: "#94A3B8", label: "下書き" },
  sent: { bg: "#DBEAFE", text: "#1E40AF", label: "送付済" },
  paid: { bg: "#D1FAE5", text: "#065F46", label: "入金済" },
  overdue: { bg: "#FEE2E2", text: "#991B1B", label: "期限超過" },
};
const StatusBadge = ({ status }) => {
  const s = INVOICE_STATUS[status] || { bg: "#334155", text: "#94A3B8", label: status };
  return <span style={{ background: s.bg, color: s.text, padding: "2px 10px", borderRadius: 20, fontSize: 11, fontWeight: 600 }}>{s.label}</span>;
};

/* ── UI ── */
const Card = ({ children, style = {}, onClick }) => (
  <div onClick={onClick} style={{ background: "#1E293B", borderRadius: 12, padding: 24, border: "1px solid #334155", ...style }}>{children}</div>
);
const PageTitle = ({ children, sub, right }) => (
  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 28, flexWrap: "wrap", gap: 12 }}>
    <div><h1 style={{ fontSize: 22, fontWeight: 700, color: "#F1F5F9" }}>{children}</h1>
    {sub && <p style={{ color: "#64748B", fontSize: 13, marginTop: 4 }}>{sub}</p>}</div>{right}
  </div>
);
const Btn = ({ children, onClick, variant = "primary", disabled, style = {} }) => {
  ...
  return <button onClick={onClick} disabled={disabled} style={{ padding: "8px 18px", borderRadius: 8, border: "none", cursor: disabled ? "default" : "pointer", fontSize: 13, fontWeight: 600, opacity: disabled ? 0.5 : 1, ...v[variant], ...style }}>{children}</button>;
};
...
export default function App() {
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#0F172A", fontFamily: "'Noto Sans JP',sans-serif", color: "#E2E8F0" }}>
      ...
      {user && profile && <MainLayout profile={profile} setProfile={setProfile} user={user} page={page} setPage={setPage} logout={() => signOut(auth)} showToast={showToast} />}
    </div>
  );
}
...
function AuthPage({ page, setPage, showToast }) {
  const [email, setEmail] = useState(""); const [password, setPassword] = useState(""); const [companyName, setCompanyName] = useState(""); const [busy, setBusy] = useState(false);
  ...
  const handleRegister = async () => { if (!email || !password || !companyName) return showToast("すべて入力してください", "error"); if (password.length < 6) return showToast("パスワードは6文字以上", "error"); setBusy(true); try { const cred = await createUserWithEmailAndPassword(auth, email, password); await setDoc(doc(db, "users", cred.user.uid), { uid: cred.user.uid, email, companyName, role: "owner", createdAt: serverTimestamp(), fiscalYearStart: 4 }); await setDoc(doc(db, "settings", `accounts_${cred.user.uid}`), { list: DEFAULT_ACCOUNTS }); showToast("登録完了！"); } catch (e) { showToast("登録失敗: " + e.code, "error"); } setBusy(false); };
  return (
      ...
      <div style={{ width: "100%", maxWidth: 420, animation: "fadeIn .5s ease" }}>
        ...
        <div style={{ background: "#1E293B", borderRadius: 16, padding: 32, border: "1px solid #334155" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
            {page === "register" && <div><label style={{ fontSize: 12, color: "#94A3B8", marginBottom: 6, display: "block" }}>会社名 / 屋号</label><input style={inputBase} placeholder="株式会社サンプル" value={companyName} onChange={e => setCompanyName(e.target.value)} /></div>}
...
/* ══════════════════ LAYOUT ══════════════════ */
function MainLayout({ profile, setProfile, user, page, setPage, logout, showToast }) {
  const nav = [{ id: "dashboard", icon: "📊", label: "ダッシュボード" },{ id: "journal", icon: "📒", label: "仕訳入力" },{ id: "ledger", icon: "📖", label: "仕訳帳" },{ id: "invoices", icon: "📄", label: "請求書" },{ id: "reports", icon: "📈", label: "レポート" },{ id: "receipts", icon: "🧾", label: "領収書" },{ id: "accounts", icon: "🏷️", label: "勘定科目" },{ id: "settings", icon: "⚙️", label: "設定" }];
  return (
    <div style={{ display: "flex", minHeight: "100vh" }}>
      <div style={{ width: 220, background: "#1E293B", borderRight: "1px solid #334155", display: "flex", flexDirection: "column", flexShrink: 0, position: "sticky", top: 0, height: "100vh", overflowY: "auto" }}>
        ...
        <div style={{ padding: "14px 12px", borderTop: "1px solid #334155" }}><div style={{ fontSize: 11, color: "#64748B", marginBottom: 6, padding: "0 6px" }}>{profile.email}</div><button onClick={logout} style={{ width: "100%", padding: "7px 10px", borderRadius: 6, border: "1px solid #334155", background: "transparent", color: "#EF4444", fontSize: 12, cursor: "pointer" }}>ログアウト</button></div>
      </div>
...
function Dashboard({ user, profile, setPage }) {
  ...
  useEffect(() => { (async () => { const eS = await getDocs(query(collection(db, "entries"), where("uid", "==", user.uid))); const eA = []; eS.forEach(d => eA.push({ id: d.id, ...d.data() })); setEntries(eA); const iS = await getDocs(query(collection(db, "invoices"), where("uid", "==", user.uid))); const iA = []; iS.forEach(d => iA.push({ id: d.id, ...d.data() })); setInvoices(iA); setLoading(false); })(); }, [user]);
  ...
  const stats = [{ label: "今月の収入", value: fmtYen(inc), color: "#10B981", icon: "↑" },{ label: "今月の支出", value: fmtYen(exp), color: "#F59E0B", icon: "↓" },{ label: "今月の利益", value: fmtYen(profit), color: profit >= 0 ? "#3B82F6" : "#EF4444", icon: profit >= 0 ? "📈" : "📉" },{ label: "未入金", value: fmtYen(unpaidTotal), color: "#8B5CF6", icon: "📄", sub: `${unpaid.length}件` }];
  return (<div><PageTitle sub={`${profile.companyName} — ${fmtMonth(now)}`}>ダッシュボード</PageTitle>
    <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 14, marginBottom: 24 }}>{stats.map((s, i) => <Card key={i} style={{ padding: 20 }}><div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}><span style={{ fontSize: 12, color: "#94A3B8" }}>{s.label}</span><span style={{ fontSize: 18 }}>{s.icon}</span></div><div style={{ fontSize: 22, fontWeight: 700, color: s.color, fontFamily: "'Space Mono',monospace" }}>{s.value}</div>{s.sub && <div style={{ fontSize: 11, color: "#64748B", marginTop: 4 }}>{s.sub}</div>}</Card>)}</div>
...
function JournalEntry({ user, showToast, setPage }) {
  ...
  useEffect(() => { (async () => { const s = await getDoc(doc(db, "settings", `accounts_${user.uid}`)); if (s.exists()) setAccounts(s.data().list || []); })(); }, [user]);
  const filtered = accounts.filter(a => a.type === type); const taxAmount = Math.floor((parseInt(amount) || 0) * taxRate / (100 + taxRate));
  const handleSubmit = async () => { if (!accountId || !amount || !date) return showToast("必須項目を入力してください", "error"); setBusy(true); const acc = accounts.find(a => a.id === accountId); await setDoc(doc(db, "entries", uid()), { uid: user.uid, type, accountId, accountName: acc?.name || "", amount: parseInt(amount), date, description, taxRate, taxAmount, createdAt: serverTimestamp() }); showToast("仕訳を登録しました！"); setAmount(""); setDescription(""); setAccountId(""); setBusy(false); };
...
function Ledger({ user, showToast }) {
  ...
  const load = useCallback(async () => { setLoading(true); const s = await getDocs(query(collection(db, "entries"), where("uid", "==", user.uid))); const a = []; s.forEach(d => a.push({ id: d.id, ...d.data() })); setEntries(a.sort((a, b) => (b.date || "").localeCompare(a.date || ""))); setLoading(false); }, [user]);
  useEffect(() => { load(); }, [load]);
  ...
  const del = async (id) => { await deleteDoc(doc(db, "entries", id)); showToast("削除しました"); load(); };
  return (<div><PageTitle sub="記録された仕訳の一覧" right={<input type="month" style={{ ...inputBase, width: "auto", colorScheme: "dark" }} value={filterMonth} onChange={e => setFilterMonth(e.target.value)} />}>仕訳帳</PageTitle>
    ...
    {loading ? <p style={{ color: "#64748B" }}>読み込み中...</p> : filtered.length === 0 ? <Card><p style={{ color: "#475569", textAlign: "center", padding: 20 }}>この月の仕訳はありません</p></Card> : <Card style={{ padding: 0, overflow: "hidden" }}><table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}><thead><tr style={{ background: "#0F172A" }}>{["日付","区分","勘定科目","摘要","金額",""].map(h => <th key={h} style={{ padding: "10px 14px", textAlign: "left", color: "#64748B", fontWeight: 500, fontSize: 12 }}>{h}</th>)}</tr></thead><tbody>{filtered.map(e => <tr key={e.id} style={{ borderTop: "1px solid #334155" }}><td style={{ padding: "10px 14px", color: "#94A3B8", fontFamily: "'Space Mono',monospace", fontSize: 12 }}>{e.date}</td><td style={{ padding: "10px 14px" }}><span style={{ fontSize: 11, fontWeight: 600, padding: "2px 8px", borderRadius: 4, background: e.type === "income" ? "rgba(16,185,129,.15)" : "rgba(245,158,11,.15)", color: e.type === "income" ? "#10B981" : "#F59E0B" }}>{e.type === "income" ? "収入" : "支出"}</span></td><td style={{ padding: "10px 14px", color: "#E2E8F0" }}>{e.accountName}</td><td style={{ padding: "10px 14px", color: "#94A3B8" }}>{e.description || "—"}</td><td style={{ padding: "10px 14px", fontWeight: 600, fontFamily: "'Space Mono',monospace", color: e.type === "income" ? "#10B981" : "#F59E0B" }}>{fmtYen(e.amount)}</td><td style={{ padding: "10px 14px" }}><button onClick={() => del(e.id)} style={{ background: "none", border: "none", color: "#64748B", cursor: "pointer", fontSize: 14 }}>✕</button></td></tr>)}</tbody></table></Card>}
  </div>);
}
...
function InvoicesPage({ user, profile, showToast }) {
  ...
  const load = useCallback(async () => { setLoading(true); const s = await getDocs(query(collection(db, "invoices"), where("uid", "==", user.uid))); const a = []; s.forEach(d => a.push({ id: d.id, ...d.data() })); setInvoices(a.sort((a, b) => (b.createdAt?.seconds || 0) - (a.createdAt?.seconds || 0))); setLoading(false); }, [user]);
  useEffect(() => { load(); }, [load]);
...
function Reports({ user }) {
  ...
  useEffect(() => { (async () => { const s = await getDocs(query(collection(db, "entries"), where("uid", "==", user.uid))); const a = []; s.forEach(d => a.push({ id: d.id, ...d.data() })); setEntries(a); setLoading(false); })(); }, [user]);
  ...
  const monthlyPL = useMemo(() => { const d = []; for (let m = 1; m <= 12; m++) { const mk2 = `${year}-${m.toString().padStart(2,"0")}`; const me = ye.filter(e => e.date && e.date.startsWith(mk2)); d.push({ month: m, income: me.filter(e => e.type === "income").reduce((s, e) => s + (e.amount || 0), 0), expense: me.filter(e => e.type === "expense").reduce((s, e) => s + (e.amount || 0), 0), profit: 0 }); d[d.length-1].profit = d[d.length-1].income - d[d.length-1].expense; } return d; }, [ye, year]);
  if (loading) return <p style={{ color: "#64748B" }}>読み込み中...</p>;
...
function Receipts({ user, showToast }) {
  ...
  const load = useCallback(async () => { setLoading(true); const s = await getDocs(query(collection(db, "receipts"), where("uid", "==", user.uid))); const a = []; s.forEach(d => a.push({ id: d.id, ...d.data() })); setReceipts(a.sort((a, b) => (b.uploadedAt?.seconds || 0) - (a.uploadedAt?.seconds || 0))); setLoading(false); }, [user]);
  useEffect(() => { load(); }, [load]);

  const handleUpload = async (e) => {
    const file = e.target.files[0]; if (!file) return;
    if (file.size > 10 * 1024 * 1024) return showToast("10MB以下にしてください", "error");
    setUploading(true);
    try {
      const result = await gasUpload(file);
      const id = uid();
      await setDoc(doc(db, "receipts", id), {
        uid: user.uid, name: file.name, size: result.size || file.size, type: file.type,
        url: result.fileUrl, thumbnailUrl: result.thumbnailUrl || result.fileUrl,
        driveFileId: result.fileId, memo, uploadedAt: serverTimestamp(),
      });
      showToast("領収書を保存しました！"); setMemo(""); load();
    } catch (err) { showToast("アップロード失敗: " + err.message, "error"); }
    setUploading(false); e.target.value = "";
  };

  const del = async (r) => {
    if (r.driveFileId) { try { await gasDelete(r.driveFileId); } catch {} }
    await deleteDoc(doc(db, "receipts", r.id));
    showToast("削除しました"); load();
  };
  const fmtSize = (b) => b < 1024 ? b + " B" : b < 1048576 ? (b / 1024).toFixed(1) + " KB" : (b / 1048576).toFixed(1) + " MB";

  return (<div><PageTitle sub="領収書・レシートの保存（Google Drive）">領収書</PageTitle>
    <Card style={{ marginBottom: 20 }}><div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
      <div><label style={{ fontSize: 12, color: "#94A3B8", marginBottom: 6, display: "block" }}>メモ（任意）</label><input style={inputBase} placeholder="例: タクシー代、文具購入..." value={memo} onChange={e => setMemo(e.target.value)} /></div>
      <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
        <label style={{ padding: "10px 20px", borderRadius: 8, cursor: uploading ? "default" : "pointer", background: "linear-gradient(135deg,#10B981,#059669)", color: "#fff", fontSize: 13, fontWeight: 600, opacity: uploading ? .6 : 1 }}>
          {uploading ? "アップロード中..." : "🧾 ファイルを選択"}<input type="file" accept="image/*,.pdf" style={{ display: "none" }} onChange={handleUpload} disabled={uploading} />
        </label><span style={{ fontSize: 12, color: "#64748B" }}>画像・PDF（最大10MB）→ Google Drive に保存</span>
      </div>
    </div></Card>
    {loading ? <p style={{ color: "#64748B" }}>読み込み中...</p> : receipts.length === 0 ? <Card><p style={{ color: "#475569", textAlign: "center", padding: 20 }}>領収書がありません</p></Card>
      : <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(220px,1fr))", gap: 14 }}>
          {receipts.map(r => <Card key={r.id} style={{ padding: 0, overflow: "hidden" }}>
            {r.type?.startsWith("image/") ? <a href={r.url} target="_blank" rel="noreferrer"><div style={{ height: 140, background: `url(${r.thumbnailUrl || r.url}) center/cover`, borderBottom: "1px solid #334155" }} /></a>
              : <a href={r.url} target="_blank" rel="noreferrer" style={{ display: "flex", alignItems: "center", justifyContent: "center", height: 140, background: "#0F172A", borderBottom: "1px solid #334155", color: "#64748B", textDecoration: "none", fontSize: 32 }}>📄</a>}
            <div style={{ padding: 14 }}>
              <div style={{ fontSize: 13, fontWeight: 500, color: "#E2E8F0", marginBottom: 4, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{r.memo || r.name}</div>
              <div style={{ fontSize: 11, color: "#64748B" }}>{fmtDate(r.uploadedAt)} · {fmtSize(r.size)}</div>
              <button onClick={() => del(r)} style={{ background: "none", border: "none", color: "#EF4444", cursor: "pointer", fontSize: 11, marginTop: 6 }}>削除</button>
            </div>
          </Card>)}
        </div>}
  </div>);
...
function AccountsPage({ user, showToast }) {
  ...
  useEffect(() => { (async () => { const s = await getDoc(doc(db, "settings", `accounts_${user.uid}`)); if (s.exists()) setAccounts(s.data().list || []); })(); }, [user]);
  const save = async (list) => { await setDoc(doc(db, "settings", `accounts_${user.uid}`), { list }); setAccounts(list); };
  const add = async () => { if (!newAcc.name) return showToast("科目名を入力してください", "error"); await save([...accounts, { ...newAcc, id: uid() }]); setNewAcc({ name: "", type: "expense", group: "販管費" }); showToast("追加しました！"); };
  const remove = async (id) => { await save(accounts.filter(a => a.id !== id)); showToast("削除しました"); };
  const Section = ({ title, items, color }) => <div style={{ marginBottom: 24 }}><h3 style={{ fontSize: 14, fontWeight: 600, color, marginBottom: 10, display: "flex", alignItems: "center", gap: 6 }}><span style={{ width: 8, height: 8, borderRadius: "50%", background: color }} />{title}</h3><div style={{ display: "flex", flexDirection: "column", gap: 6 }}>{items.map(a => <div key={a.id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "8px 14px", background: "#0F172A", borderRadius: 6 }}><div><span style={{ fontSize: 14, color: "#E2E8F0" }}>{a.name}</span><span style={{ fontSize: 11, color: "#64748B", marginLeft: 8 }}>{a.group}</span></div><button onClick={() => remove(a.id)} style={{ background: "none", border: "none", color: "#64748B", cursor: "pointer", fontSize: 13 }}>✕</button></div>)}</div></div>;
  return (<div><PageTitle sub="勘定科目の追加・管理">勘定科目</PageTitle><Card style={{ marginBottom: 20 }}><Section title="収入科目" items={accounts.filter(a => a.type === "income")} color="#10B981" /><Section title="経費科目" items={accounts.filter(a => a.type === "expense")} color="#F59E0B" /></Card><Card><h3 style={{ fontSize: 14, fontWeight: 600, color: "#F1F5F9", marginBottom: 12 }}>科目を追加</h3><div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}><input style={{ ...inputBase, flex: 2, minWidth: 140 }} placeholder="科目名" value={newAcc.name} onChange={e => setNewAcc({ ...newAcc, name: e.target.value })} /><select style={{ ...inputBase, flex: 1, minWidth: 100, cursor: "pointer" }} value={newAcc.type} onChange={e => setNewAcc({ ...newAcc, type: e.target.value })}><option value="income">収入</option><option value="expense">経費</option></select><input style={{ ...inputBase, flex: 1, minWidth: 100 }} placeholder="分類" value={newAcc.group} onChange={e => setNewAcc({ ...newAcc, group: e.target.value })} /><Btn onClick={add}>追加</Btn></div></Card></div>);
}

/* ══════════════════ SETTINGS ══════════════════ */
function SettingsPage({ user, profile, setProfile, showToast }) {
  const [companyName, setCompanyName] = useState(profile.companyName || ""); const [fiscalYearStart, setFiscalYearStart] = useState(profile.fiscalYearStart || 4);
  const save = async () => { const u = { ...profile, companyName, fiscalYearStart }; await updateDoc(doc(db, "users", user.uid), { companyName, fiscalYearStart }); setProfile(u); showToast("保存しました！"); };
  return (<div><PageTitle sub="アカウント設定">設定</PageTitle><Card><div style={{ display: "flex", flexDirection: "column", gap: 18 }}><div><label style={{ fontSize: 12, color: "#94A3B8", marginBottom: 6, display: "block" }}>会社名 / 屋号</label><input style={inputBase} value={companyName} onChange={e => setCompanyName(e.target.value)} /></div><div><label style={{ fontSize: 12, color: "#94A3B8", marginBottom: 6, display: "block" }}>メールアドレス</label><input style={{ ...inputBase, opacity: .6 }} value={profile.email} disabled /></div><div><label style={{ fontSize: 12, color: "#94A3B8", marginBottom: 6, display: "block" }}>会計年度の開始月</label><select style={{ ...inputBase, cursor: "pointer" }} value={fiscalYearStart} onChange={e => setFiscalYearStart(+e.target.value)}>{Array.from({ length: 12 }, (_, i) => i + 1).map(m => <option key={m} value={m}>{m}月</option>)}</select></div><Btn onClick={save} style={{ alignSelf: "flex-end" }}>保存する</Btn></div></Card><Card style={{ marginTop: 16 }}><div style={{ fontSize: 12, color: "#64748B" }}><div>UID: <span style={{ color: "#94A3B8", fontFamily: "'Space Mono',monospace" }}>{user.uid}</span></div><div style={{ marginTop: 4 }}>登録日: {fmtDate(profile.createdAt)}</div></div></Card></div>);
}
